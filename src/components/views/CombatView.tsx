...
export function CombatView() {
    ...
    return (
        <div className="pb-24 space-y-4 animate-fade-in">
            ...
            <div className="card-parchment p-4 stagger-3 animate-slide-up">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        <Swords size={18} className="text-white" />
                        <div>
                            <h3 className="font-display text-sm text-parchment tracking-wider uppercase">Combat Status</h3>
                            <div className="text-[10px] font-japanese text-parchment/80 -mt-1 opacity-50 tracking-widest">戦闘状況</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-card-elevated p-3 rounded-lg border border-white/5 shadow-inner">
                        <div className="text-[10px] text-parchment/70 uppercase tracking-wider mb-1">Round</div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="bg-card-elevated p-3 rounded-lg border border-white/5 shadow-inner">
                        <div className="text-[10px] text-parchment/70 uppercase tracking-wider mb-1">Turn Count</div>
            ...
            {/* Vitals Card */}
            <div className="card-parchment p-4 stagger-4 animate-slide-up">
                <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-card-elevated p-3 rounded-lg border border-white/5">
                        <div className="text-[10px] text-parchment/70 uppercase tracking-wider">HP</div>
                        <div className="text-lg font-display text-white">{character.hp.current}/{character.hp.max}</div>
                    </div>
                    <div className="bg-card-elevated p-3 rounded-lg border border-white/5">
                        <div className="text-[10px] text-parchment/70 uppercase tracking-wider">Temp</div>
                        <div className="text-lg font-display text-accent-glow">{character.hp.temp}</div>
                    </div>
                    <div className="bg-card-elevated p-3 rounded-lg border border-white/5">
                        <div className="text-[10px] text-parchment/70 uppercase tracking-wider">AC</div>
                ...
                <div className="mt-4 grid grid-cols-1 gap-3">
                    <div className="flex gap-2">
                        ...
                        />
                        <button className="btn-primary px-4 bg-vermillion/20 hover:bg-vermillion/40 text-white min-h-[48px] border-vermillion/30" onClick={handleDamage} aria-label="Take damage">Take</button>
                    </div>
                    <div className="flex gap-2">
                        <input
                            type="number"
                        ...
                        />
                        <button className="btn-primary px-4 bg-moss/20 hover:bg-moss/40 text-white min-h-[48px] border-moss/30" onClick={handleHeal} aria-label="Heal">Heal</button>
                    </div>
                    <div className="flex gap-2">
                        <input
                            type="number"
                            min={0}
                            placeholder="Temp HP"
                            className="flex-1 bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white placeholder:text-parchment/60"
                            value={tempInput}
                            onChange={(e) => setTempInput(e.target.value)}
                            aria-label="Temporary HP"
                        />
                        <button className="btn-primary px-4" onClick={handleTempHP} aria-label="Set temporary HP">Set</button>
                    </div>
                </div>
            </div>

            {/* Conditions (Simplified) */}
            <div className="card-parchment p-4 space-y-3 stagger-5 animate-slide-up">
                <div className="flex items-center gap-2">
                ...
                {combat.conditions.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-2">
                        {combat.conditions.map(c => (
                            <button key={c} onClick={() => dispatch(conditionRemoved(c))} className="px-3 py-2 rounded-lg text-[10px] uppercase tracking-wider bg-accent/10 text-accent border border-accent/30 flex items-center gap-1" aria-label={`Remove ${c}`}>
                                {c} <X size={10} />
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex gap-2">
                    ...
                    />
                    <button className="btn-primary px-4" onClick={() => handleAddCondition(conditionInput)} aria-label="Add condition">Add</button>
                </div>
                    ...
                    {QUICK_CONDITIONS.map(c => (
                        <button
                            ...
                            className={`px-3 py-3 rounded-xl text-[10px] uppercase tracking-wider border transition-all min-h-[48px] flex-1 min-w-[30%]
                                ${combat.conditions.includes(c) ? 'bg-accent text-bg-dark border-accent' : 'bg-white/5 text-parchment/70 border-white/10 hover:border-white/30'}
                            `}
                            aria-label={`Add condition: ${c}`}
                        >
                            {c}
                        </button>
                    ))}
            ...
            <div className="space-y-4 animate-slide-up">
                ...
                <div className="card-parchment p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <button
                            className={`px-3 py-3 rounded-xl text-xs uppercase tracking-wider border transition-all min-h-[48px] ${combat.reactionAvailable ? 'bg-moss/20 text-moss border-moss/40' : 'bg-white/5 text-parchment/70 border-white/10'}`}
                            onClick={() => dispatch(reactionToggled())}
                            aria-label={`Reaction ${combat.reactionAvailable ? 'Ready' : 'Spent'}`}
                        >
                            Reaction {combat.reactionAvailable ? 'Ready' : 'Spent'}
                        </button>
                        <button
                            className={`px-3 py-3 rounded-xl text-xs uppercase tracking-wider border transition-all min-h-[48px] ${combat.bonusActionAvailable ? 'bg-moss/20 text-moss border-moss/40' : 'bg-white/5 text-parchment/70 border-white/10'}`}
                            onClick={() => dispatch(bonusActionToggled())}
                            aria-label={`Bonus Action ${combat.bonusActionAvailable ? 'Ready' : 'Spent'}`}
                        >
            ...
            {/* Combat Log */}
            <div className="card-parchment p-4 space-y-3 animate-slide-up">
                <div className="flex items-center gap-2">
                    <Bolt size={18} className="text-white" />
                    <div>
                        <h3 className="font-display text-sm text-parchment tracking-wider uppercase">Combat Log</h3>
                        <div className="text-[10px] font-japanese text-parchment/80 -mt-1 opacity-50 tracking-widest">戦闘記録</div>
                    </div>
                </div>
                <div className="flex gap-2">
                    <input type="text" placeholder="Quick note" className="flex-1 bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm text-white" value={noteInput} onChange={e => setNoteInput(e.target.value)} aria-label="Combat note" />
                    ...
                    }} aria-label="Log note">Log</button>
                </div>
                <div className="space-y-2 max-h-64 overflow-y-auto" role="log" aria-live="polite">
                    {combat.log.slice(0, 50).map(entry => (
                        <div key={entry.id} className="bg-card-elevated/70 border border-white/10 rounded-lg p-3">
                            <div className="text-[10px] text-parchment/70 uppercase tracking-wider">{new Date(entry.timestamp).toLocaleTimeString()}</div>
                            <div className="text-sm text-parchment-light">{entry.title}</div>
                            {entry.detail && <div className="text-xs text-parchment/70 mt-1">{entry.detail}</div>}
                        </div>
                    ))}
                </div>
            </div>
